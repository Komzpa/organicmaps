...
private:
  /// Loads member geometries from the MWM identified by an alive @p guard. Each
  /// returned Geometry has a single RelationRef seeded with @p relID.
  ...
  std::vector<Geometry> LoadMemberGeometries(feature::RouteRelation const & relation, FeaturesLoaderGuard const & guard,
                                             RelationID const & relID, size_t & startIdx);
  std::vector<Geometry> LoadMemberGeometries(feature::RouteRelation const & relation, FeaturesLoaderGuard const & guard,
                                             RelationID const & relID)
  {
    size_t dummyIdx;
    return LoadMemberGeometries(relation, guard, relID, dummyIdx);
  }
  ...
  void AppendNeighbourMembers(MwmSet::MwmId const & mwmId, uint32_t osmRelID, std::vector<Geometry> & members);
